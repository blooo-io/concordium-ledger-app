...
int formatReleaseTime(uint64_t release_time, uint8_t *dst, size_t dstLength) {
    uint64_t seconds = release_time / 1000;
    tm time = {0};
    if (secondsToTm(seconds, &time) != 0) {
        // TODO: add a dedicated errer code
        io_send_sw(SW_BAD_STATE);
    }
    return timeToDisplayText(time, dst, dstLength);
}
